...
Interrupt sensitivity has three options: **low** (default), **medium**, and **high**.

In summary:
- **Possible values:** low, medium, high
- **Default value:** low
- **low** = ignore short utterances (≤3 words) and backchannel phrases (e.g. "sure", "uh-huh")
- **medium** = only ignore known backchannel phrases, but short meaningful words like "wait" will interrupt
- **high** = any word from the human triggers an interruption
